        ...
        if (window.currentFilter === "Search" && window.hasSearched) {
            ...
            if (alreadyExists) {
                const applyScript = `
                    ...
                    pkill -CONT -x mpvpaper 2>/dev/null; pkill -TERM -x mpvpaper 2>/dev/null; for _k in 1 2 3 4 5 6 7 8; do pgrep -x mpvpaper >/dev/null || break; sleep 0.1; done; pkill -KILL -x mpvpaper 2>/dev/null; true
        ...
        if (isVideo) {
            wallpaperCmd = `
                ...
                if [ "${escOutputs}" = "all" ]; then
                    mpvpaper -o 'loop --no-audio --hwdec=vaapi --profile=fast --vd-lavc-threads=1 --cache=no --no-osc --no-osd-bar --no-input-default-bindings --no-input-cursor' '*' "${escOriginal}" >> ${logFile} 2>&1 &
                else
                    IFS=',' read -ra MON_ARR <<< "${escOutputs}"
                    for mon in "\${MON_ARR[@]}"; do
                        mpvpaper -o 'loop --no-audio --hwdec=vaapi --profile=fast --vd-lavc-threads=1 --cache=no --no-osc --no-osd-bar --no-input-default-bindings --no-input-cursor' "\$mon" "${escOriginal}" >> ${logFile} 2>&1 &
                    done
                fi
            `;
        } else {
