...
StyledRect {
    ...
    // ── BITE-OS: low-battery watcher ─────────────────────────────────────
    // Fires a toast recommending Saver mode when the battery drops past
    // 20% / 10% / 5% while on battery. Each threshold fires once per shell
    // session and rearms once you're charging again.
    Item {
        id: batteryWatcher

        property int lastNotified: 100  // start above any threshold
        ...
        Connections {
            ...
            function onPercentageChanged(): void {
                ...
                for (const t of thresholds) {
                    if (perc <= t && batteryWatcher.lastNotified > t) {
                        batteryWatcher.lastNotified = t;
                        const urgency = t <= 10 ? "critical" : "normal";
                        Quickshell.execDetached(["notify-send",
                            "-a", "GLITCH BITE",
                            "-i", "battery-low-symbolic",
                            "-u", urgency,
                            "Battery " + perc + "%",
                            "Tap the 🍃 leaf in the battery menu to switch to Saver mode and squeeze more time out."]);
                        break;
                    }
                }
            }
        ...
        Connections {
            ...
            function onOnBatteryChanged(): void {
                if (!UPower.onBattery)
                    batteryWatcher.lastNotified = 100;  // rearm on plug-in
            }
        }
    }
}
