...
| File | What it does |
|---|---|
| `dots-switch.sh` | Hot-swaps between the two complete desktops (`caelestia` ⇄ `serpantinum`). Backs up the live config before every swap and arms a 30-second watchdog that auto-reverts if the new desktop fails to come up — so you can't get locked out. |
